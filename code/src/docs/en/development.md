...
If you change package.json so it is no longer in sync with the package-lock.json, building of the application will fail, because the `ci` command used to install packages safely will not allow this.

If you update package.json on purpose, you should run `make install_new_dependencies` before building the application again.
